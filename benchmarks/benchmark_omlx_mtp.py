...
async def get_session_cookie(base_url: str, api_key: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        ...
        if "omlx_admin_session=" in cookie_header:
            return cookie_header.split("omlx_admin_session=")[1].split(";")[0]
...
async def update_settings(base_url: str, model_id: str, cookie_value: str, settings_payload: dict) -> None:
    ...
    headers = {"Cookie": f"omlx_admin_session={cookie_value}", "Content-Type": "application/json"}
    async with httpx.AsyncClient() as client:
...
async def unload_model(base_url: str, model_id: str, cookie_value: str) -> None:
    ...
    headers = {"Cookie": f"omlx_admin_session={cookie_value}"}
    async with httpx.AsyncClient() as client:
...
async def run_query(base_url: str, model_id: str, prompt_text: str, is_warmup: bool = False) -> dict:
    ...
    try:
        async with (
            ...
            client.stream("POST", completion_url, headers=headers, json=payload) as response,
        ):
            if response.status_code != 200:
...
async def run_benchmark_for_config(
    base_url: str, model_id: str, cookie_value: str, config_name: str, settings_payload: dict
) -> dict:
    print("\n" + "=" * 60)
    ...
    print(
        f"\nWarm-up completed (Loaded model in "
        f"{warmup_result['prefill_duration']:.2f}s, generated "
        f"{warmup_result['token_count']} tokens)"
    )
...
async def main():
    ...
    configurations = [
        {
            ...
            "settings": {"mtp_enabled": True, "turboquant_kv_enabled": False, "dflash_enabled": False},
        },
        {
            "name": "Standard Baseline (MTP Disabled)",
            "settings": {"mtp_enabled": False, "turboquant_kv_enabled": False, "dflash_enabled": False},
        },
        {
    ...
    try:
        for config_entry in configurations:
            result = await run_benchmark_for_config(
                server_base_url, model_id, cookie_value, config_entry["name"], config_entry["settings"]
            )
            benchmark_results.append(result)
    finally:
        # Restore original settings
        print("\nRestoring original settings (Native MTP enabled)...")
        original_settings = {"mtp_enabled": True, "turboquant_kv_enabled": False, "dflash_enabled": False}
    ...
    for result in benchmark_results:
        if result["success"]:
            print(
                f"| {result['config']} | "
                f"{result['prefill_duration']:.4f}s | "
