...
- fossil-scm and repository operations
...
4. Database choice
- Start with ZODB for a single-server control plane
- Reason: ZODB is operationally simpler and surprisingly strong for the early and middle stages of an agent system
...
- When fossil-scm is available, prefer one worktree per parallel coding task, subtask, or machine-owned lane
...
- what capabilities exist right now: shell, filesystem, fossil-scm, browser automation, desktop automation, network, scheduling, hooks, background tasks, persistent storage, tool calling, sub-agent delegation, UI surfaces
...
If the runtime has strong shell and fossil-scm access but weak orchestration:
...
- when fossil-scm is available, default parallel coding work to one fossil worktree per owned task or per machine-owned lane
...
- worktree or branch isolation per task when fossil-scm is available
